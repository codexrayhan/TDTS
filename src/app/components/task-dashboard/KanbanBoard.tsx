...
function TaskCard({ task, people, onOpen }: { task: Task; people: Employee[]; onOpen?: (task: Task) => void }) {
  ...
  return <motion.div ref={drag as unknown as Ref<HTMLDivElement>} onClick={() => onOpen?.(task)} whileHover={{ y: -2, boxShadow: "0 8px 20px rgba(0,0,0,0.08)" }} className={`rounded-xl border border-border-primary bg-surface-bg p-3 shadow-md hover:shadow-lg transition-shadow cursor-pointer`} style={{ opacity: dragging ? 0.45 : 1 }}>
